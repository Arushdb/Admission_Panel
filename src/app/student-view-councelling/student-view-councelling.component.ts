...
export class StudentViewCouncellingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log("Arush mode", this.myvariable);
  }
  ...
  loadProgramCombo(appNo) {
    this.myservice.getOwnerProgram2(appNo).subscribe((data) => {
  ...
  OwnerProgramChange(val) {
    ...
    if (this.prog != undefined) {
      ...
      this.myservice
        .getStdentInfoforViewCouncelling(this.Application,this.myvariable)
        .subscribe((responce) => {
          this.studentInfo = Array.from(
