...
export class CounsellingComponent implements OnInit {
  ...
  getCourseDetail(val) {
    this.myService.getStudentMarksforCounselling(val).subscribe((data) => {
  ...
  verify() {
    debugger;
    this.openConfirmDiaog(
      "I checked all component marks, all entered marks are correct , record is verified."
    )
      .afterClosed()
      .subscribe((res) => {
        if (res == true) {
          this.myService
            .updateFinalCandidate(this.regNo, "Record VERIFIED", "VERIFIED")
            .subscribe((data) => {
              // this. getCourseDetail(this.Appno);
  ...
  showDummyImage() {
    this.getBase64ImageFromURL(this.imageUrl + "BLANK" + ".jpg").subscribe(
      (base64data) => {
  ...
  showImage() {
    debugger;
    this.getBase64ImageFromURL(this.imageUrl + this.Appno + ".jpeg").subscribe(
      (base64data) => {
        console.log(base64data);
        debugger;
        this.base64Image = "data:image/jpg;base64," + base64data;
